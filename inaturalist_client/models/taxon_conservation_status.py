...
class TaxonConservationStatus(BaseModel):
    ...
    source_id: Optional[StrictInt] = Field(
        default=None,
        description="Identifier for the iNat source record associated with this status, retrievable via https://www.inaturalist.org/sources/:id.json (this endpoint is not a part of our public API and is thus subject to change or removal) ",
    )
    authority: Optional[StrictStr] = Field(
        default=None, description="Organization that declared this status "
    )
    status: Optional[StrictStr] = Field(
        default=None,
        description="Body of the status, often coded, particularly when the status comes from the IUCN or NatureServe. Consult the authority and/or the status URL for details about the meanings of codes. ",
    )
    status_name: Optional[StrictStr] = Field(
        default=None, description="Human-readable name of the status if it was coded. "
    )
    iucn: Optional[StrictInt] = Field(
        default=None,
        description="Coded value representing the equivalent IUCN status. Mappings: NOT_EVALUATED = 0, DATA_DEFICIENT = 5, LEAST_CONCERN = 10, NEAR_THREATENED = 20, VULNERABLE = 30, ENDANGERED = 40, CRITICALLY_ENDANGERED = 50, EXTINCT_IN_THE_WILD = 60, EXTINCT = 70 ",
    )
    geoprivacy: Optional[StrictStr] = Field(
        default=None,
        description="Default geoprivacy for observations of this taxon in the status's place. ",
    )
    place: Optional[CorePlace] = None
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of TaxonConservationStatus from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of TaxonConservationStatus from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                ...
                "place": CorePlace.from_dict(obj["place"])
                if obj.get("place") is not None
                else None,
            }
        )
        return _obj
