...
class PostUser(BaseModel):
    ...
    user: Optional[PostUserUser] = None
    icon_delete: Optional[StrictBool] = Field(
        default=False, description="Set to true to remove the current user icon."
    )
    __properties: ClassVar[List[str]] = ["user", "icon_delete"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "user": PostUserUser.from_dict(obj["user"])
                if obj.get("user") is not None
                else None,
                "icon_delete": obj.get("icon_delete")
                if obj.get("icon_delete") is not None
                else False,
            }
        )
        return _obj
