...
class Message(BaseModel):
    ...
    user_id: Optional[StrictInt] = Field(
        default=None,
        description="ID of the user to whom this message belongs. Messages work like email, so the sender gets a copy and the recipient gets a copy of each message. This is always the authenticated user, so there's no real need for a full user object. ",
    )
    to_user: Optional[User] = None
    from_user: Optional[User] = None
    thread_id: Optional[StrictInt] = Field(
        default=None,
        description="Identifier for the message thread, generally the ID of the sender's copy of the first message ",
    )
    thread_messages_count: Optional[StrictInt] = Field(
        default=None,
        description="Number of messages in this thread. Only included when threads=true ",
    )
    thread_flags: Optional[List[Dict[str, Any]]] = Field(
        default=None,
        description="Array of flags on messages in this thread. Only included when threads=true ",
    )
        ...
        _obj = cls.model_validate(
            {
                ...
                "to_user": User.from_dict(obj["to_user"])
                if obj.get("to_user") is not None
                else None,
                "from_user": User.from_dict(obj["from_user"])
                if obj.get("from_user") is not None
                else None,
                "thread_id": obj.get("thread_id"),
                "thread_messages_count": obj.get("thread_messages_count"),
                "thread_flags": obj.get("thread_flags"),
            }
        )
        return _obj
